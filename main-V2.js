...
var _OutlineItem = class {
  constructor(block, editor, onUpdate, onFocus, onRender, getMultiSelectionManager, onBulletClick, getZoomedBlockId, app, sourcePath, settings, parentComponent) {
  ...
  createLivePreviewContent(contentLine) {
    ...
    this.editorElement.setCssProps({ "overflow": "hidden" });
    this.editorElement.setCssProps({ "resize": "none" });
   ...
   */
  handleKeyDown(e) {
    if (!IsolationLayer.getInstance().isElementInWorkflowyContainer(e.target)) {
      return;
    }
  ...
  showDropZone(rect, position) {
    ...
    _OutlineItem.dropZone.setCssProps({ "display": "block" });
    _OutlineItem.dropZone.setCssProps({ "top": `${top}px` });
    _OutlineItem.dropZone.setCssProps({ "left": `${left}px` });
    _OutlineItem.dropZone.setCssProps({ "width": `${width}px` });
    const dashPadding = 3;
    ...
    if (targetIndent > 0) {
      _OutlineItem.dropZonePadding.setCssProps({ "width": `${targetIndent}px` });
      _OutlineItem.dropZonePadding.setCssProps({ "margin-left": `-${targetIndent}px` });
      ...
      _OutlineItem.dropZonePadding.setCssProps({ "background-image": svgPattern });
    } else {
      _OutlineItem.dropZonePadding.setCssProps({ "width": "0" });
      _OutlineItem.dropZonePadding.setCssProps({ "background-image": "none" });
    }
  }
  handleDrop(e) {
    var _a, _b, _c;
  ...
  clearDragStyles() {
    if (_OutlineItem.dropZone) {
      _OutlineItem.dropZone.setCssProps({ "display": "none" });
    }
  ...
  async renderDisplay() {
    ...
    this.displayComponent = new import_obsidian3.Component();
    if (this.parentComponent) {
...
var WorkflowySettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName("Workflowy\u63D2\u4EF6\u8BBE\u7F6E").setHeading();
    new import_obsidian5.Setting(containerEl).setName("\u5FEB\u6377\u952E\u8BBE\u7F6E").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setName("UI\u8BBE\u7F6E").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setName("\u7F16\u8F91\u5668\u8BBE\u7F6E").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setName("\u641C\u7D22\u8BBE\u7F6E").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setName("\u62D6\u62FD\u8BBE\u7F6E").setHeading();
    ...
    new import_obsidian5.Setting(containerEl).setName("\u9AD8\u7EA7\u8BBE\u7F6E").setHeading();
